...
export default function Feed({
  ...
  useEffect(() => {
    if (!selected) return;
    fetch(`/api/comments?postId=${selected._id}`)
      .then((r) => r.json())
  ...
  const like = async (post: FeedPost) => {
    ...
    if (!response.ok)
      return setMessage(
        (await response.json()).message || "Could not like this post.",
      );
      ...
      items.map((item) =>
        item._id === post._id
          ? {
              ...item,
              likes: result.liked
                ? [...item.likes, { userName: user.userName }]
                : item.likes.filter((like) => like.userName !== user.userName),
            }
          : item,
      ),
    );
  ...
  const submitComment = async (event: FormEvent) => {
    ...
    if (!response.ok) {
      setMessage(
        result.requiresAdminAppeal
          ? `${result.message} Email: admin@example.com`
          : result.message || "Could not publish comment.",
      );
  ...
  const likeComment = async (comment: Comment) => {
    ...
    const update = (item: Comment): Comment =>
  ...
  const postAction = async (
    ...
    const response = await fetch(url, {
      ...
      headers:
        action === "archive"
          ...
          : undefined,
      body: action === "archive" ? JSON.stringify({ action }) : undefined,
    });
    const data = await response.json();
    if (!response.ok)
      return toast(data.message || "Could not update post", "error");
    if (action === "delete" || action === "archive")
      ...
      setPosts((items) =>
        items.map((item) =>
          item._id === post._id
            ? ({ ...item, isPinned: data.isPinned } as FeedPost)
            : item,
        ),
      );
    setMenuPostId(null);
  ...
  const interactWithStory = async (
    type: "like" | "reaction" | "reply",
    value?: string,
  ) => {
    if (!story) return;
    const response = await fetch(`/api/stories/${story._id}/interact`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (type === "like") {
      ...
      toast(data.liked ? "Story liked" : "Like removed");
    } else {
      setStoryReply("");
      toast("Sent in chat");
    }
  ...
  const createStory = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const response = await fetch("/api/stories", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mediaUrl, mediaType: "image" }),
    });
    const result = await response.json();
    if (!response.ok)
      return toast(result.message || "Could not add story", "error");
  ...
  return (
    <main
      className={`mx-auto w-full max-w-[620px] px-3 py-5 ${darkMode ? "text-white" : "text-gray-900"}`}
    >
      {message && (
        <div
          role="alert"
          className="mb-3 flex items-center justify-between rounded-xl bg-red-50 p-3 text-sm text-red-700"
        >
          <span>{message}</span>
      ...
      {showStories && (
        <>
          ...
          <section className="mb-5 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
            <p className="mb-3 text-sm font-semibold text-gray-800">Stories</p>
            <div className="flex gap-3 overflow-x-auto pb-1">
              <button
                onClick={() => fileInput.current?.click()}
                className="flex w-16 shrink-0 flex-col items-center gap-1 text-xs text-gray-700"
              >
                <span className="grid h-14 w-14 place-items-center rounded-full border-2 border-dashed border-blue-500 text-2xl text-blue-500">
                  +
                </span>
                Your story
              </button>
              {Array.from(
                new Map(stories.map((item) => [item.userName, item])).values(),
              ).map((item) => (
                <button
                  key={item._id}
                ...
                >
                  <span
                    className={`h-14 w-14 overflow-hidden rounded-full border-2 p-0.5 ${item.viewed ? "border-gray-300" : "border-pink-500"}`}
                  >
                    <img
                      src={item.mediaUrl}
                      className="h-full w-full rounded-full object-cover"
                      alt={`${item.userName}'s story`}
                    />
                  </span>
      ...
      <div className="mb-4 flex gap-2 text-sm">
        <button
          ...
          className={`rounded-full px-3 py-1 ${filter === "all" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}
        >
          All
        </button>
        <button
          onClick={() => setFilter("photos")}
          className={`rounded-full px-3 py-1 ${filter === "photos" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}
        >
          Posts
        </button>
        <button
          onClick={() => setFilter("reels")}
          className={`rounded-full px-3 py-1 ${filter === "reels" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}
        >
          Reels
        </button>
      </div>
        ...
        {posts.length === 0 && (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white px-6 py-14 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Follow people or create a post to see photos, reels and videos
              here.
            </p>
          </div>
        )}
        {posts
          .filter((post) =>
            filter === "all" || filter === "reels"
              ? filter === "all" ||
                Boolean(post.media?.[0]?.isVideo || post.isVideo)
              : !Boolean(post.media?.[0]?.isVideo || post.isVideo),
          )
          .map((post) => {
            const media =
              post.media?.[0] ||
              (post.src ? { src: post.src, isVideo: post.isVideo } : null);
            ...
            return (
              <article
                key={post._id}
                className={`overflow-hidden rounded-xl border ${darkMode ? "border-gray-800 bg-gray-900" : "border-gray-200 bg-white"}`}
              >
                <div className="flex items-center justify-between px-4 py-3 text-sm font-semibold">
                  <button
                    onClick={() =>
                      router.push(`/components/profile/${post.userName}`)
                    }
                    className="flex items-center gap-2"
                  >
                    <span className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 text-xs text-white">
                  ...
                  {post.userName === user.userName && (
                    <div className="relative">
                      <button
                        onClick={() =>
                          setMenuPostId(
                            menuPostId === post._id ? null : post._id,
                          )
                        }
                      >
                        <FaEllipsisH />
                      ...
                      {menuPostId === post._id && (
                        <div className="absolute right-0 top-7 z-20 w-36 rounded-lg bg-white py-1 text-left text-sm text-gray-800 shadow-lg">
                          <button
                            className="block w-full px-3 py-2 hover:bg-gray-100"
                            onClick={() => postAction(post, "delete")}
                          >
                            Delete
                          </button>
                          <button
                            className="block w-full px-3 py-2 hover:bg-gray-100"
                            onClick={() => postAction(post, "archive")}
                          >
                            Archive
                          </button>
                          {!media?.isVideo && (
                ...
                <div className="px-4 py-3">
                  <div className="flex gap-4 text-xl">
                    <button onClick={() => like(post)} aria-label="Like post">
                      {liked ? (
      ...
      {selected && (
        ...
        >
          <section
            ...
            className={`max-h-[80vh] w-full max-w-lg rounded-t-2xl p-4 sm:rounded-2xl ${darkMode ? "bg-gray-900" : "bg-white"}`}
          >
                ...
                <div key={comment._id} className="text-sm">
                  <p>
                    <button
                      onClick={() =>
                        router.push(
                          `/components/profile/${comment.authorUserName}`,
                        )
                      }
                  ...
                  <div className="mt-1 flex gap-3 text-xs text-gray-500">
                    ...
                    <button
                      onClick={() => {
                        setReplyTo(comment);
                        setText(`@${comment.authorUserName} `);
                      }}
                    >
                      Reply
                    </button>
                  ...
                  {comment.replies?.map((reply) => (
                    <div key={reply._id} className="ml-6 mt-2">
                      <button
                        onClick={() =>
                          router.push(
                            `/components/profile/${reply.authorUserName}`,
                          )
                        }
                        className="font-semibold"
                      >
            ...
            >
              <input
                ...
                placeholder={
                  replyTo
                    ? `Reply to @${replyTo.authorUserName}`
                    : "Add a comment…"
                }
      ...
      {story && (
        <div
          className="fixed inset-0 z-[55] grid place-items-center bg-black/80 p-4"
            ...
            /> : <img
              onLoad={() => {
                if (story.userName !== user.userName) {
                  ...
                  setStories((items) =>
                    items.map((item) =>
                      item._id === story._id ? { ...item, viewed: true } : item,
                    ),
                  );
                }
              }}
            ...
            {story.userName === user.userName ? (
              <p className="mt-2 text-center text-xs text-white">
                ...
                {story.viewedBy?.length
                  ? ` · ${story.viewedBy.map((view) => `@${view.userName}`).join(", ")}`
                  : ""}
              </p>
            ) : (
              <>
                <div className="mt-3 flex justify-center gap-3">
                  <button
                    onClick={() => interactWithStory("like")}
                  ...
                  {["🔥", "😍", "😂"].map((reaction) => (
                    <button
                      key={reaction}
                      onClick={() => interactWithStory("reaction", reaction)}
                ...
                <form
                  onSubmit={(event) => {
                    event.preventDefault();
                    interactWithStory("reply", storyReply);
                  }}
                ...
                >
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-full px-4 py-2 text-sm text-black"
                  />
                  <button className="rounded-full bg-white px-4 text-sm font-semibold">
                    Send
                  </button>
                </form>
              </>
      ...
      {appealRequired && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <section className="w-full max-w-sm rounded-2xl bg-white p-5 text-gray-900 shadow-xl">
            <h2 className="font-semibold">Comment access blocked</h2>
            <p className="mt-2 text-sm text-gray-600">
              Tell our team why your account should be reviewed.
            </p>
            <div className="mt-5 flex justify-end gap-3">
              <button
                onClick={() => setAppealRequired(false)}
                className="text-sm"
              >
                Cancel
              </button>
              <a
                href={`mailto:${process.env.NEXT_PUBLIC_SUPPORT_EMAIL || "support@example.com"}?subject=Comment%20access%20appeal`}
